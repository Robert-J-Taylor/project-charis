...
export const contactFormSchema = z.object({
  ...
  helpText: z.string().min(10, 'Please provide at least 10 characters describing how we can help'),
...
export const validateContactForm = (data: unknown) => {
  ...
  if (challenge === 'Other' && (!otherChallenge || otherChallenge.trim().length === 0)) {
    return {
      success: false,
  ...
  if (!hasSubstantialHelp && !hasSubstantialExtra) {
    ...
    return {
      success: false,
      errors: { helpText: ['Please provide at least 3 characters describing how we can help or share additional thoughts'] }
    };
  }
